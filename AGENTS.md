...
本仓库文档索引：`lib_ui/docs/README.md`。索引带状态标记（`现行` / `归档`），先看状态再决定要不要读。组件公开 API 全文目前在根仓库 `docs/UI.md`，索引里有指向。

**问题排查记录（问题 → 根因 → 方案）不放本仓**，统一在工作区根 `docs/devlog/`，索引 `docs/devlog/README.md`。本仓当前唯一相关条目是 `AlertProvider` 整树重渲染（状态：未解决）。
